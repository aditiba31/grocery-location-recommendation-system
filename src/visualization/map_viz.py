# Visualizations
# Generates California zip code map, ROC curves, feature importance,
# and model comparison charts.
# Run: python src/visualization/map_viz.py
...
def plot_california_map(all_f, tj_all):
    ...
    print(f"  Plotting {len(scored)} zip codes...")

    m = folium.Map(location=[36.7783, -119.4179], zoom_start=6,
                   tiles="CartoDB positron")

    def get_color(p):
        if p >= 0.90:   return "#C0392B"
        elif p >= 0.75: return "#E74C3C"
        elif p >= 0.60: return "#E67E22"
        elif p >= 0.45: return "#F1C40F"
        elif p >= 0.30: return "#2ECC71"
        else:           return "#3498DB"
...
def plot_roc_curves():
    ...
    plt.close()
    print("  Saved ROC curves chart.")


# --- 5. Top Recommendations ---

def plot_top_recommendations(all_f, tj_all):
    print("Plotting top recommendations...")
    ...
    ax.set_title("Top 20 Recommended New Trader Joe's Locations — California\n(Zip codes with no existing TJ store)",
                 fontsize=13, fontweight="bold")
    ...
    plt.close()
    print("  Saved top recommendations chart.")


# --- 6. Demographic Profile ---

def plot_demographic_profile(all_f):
    print("Plotting demographic profile...")
    ...
    plt.close()
    print("  Saved demographic profile chart.")
