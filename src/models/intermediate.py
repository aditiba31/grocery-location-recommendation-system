...
# Run: python src/models/intermediate.py
...
def plot_auc_comparison(results):
    ...
    ax.axhline(0.8467, color="#95A5A6", linestyle="--",
               linewidth=1, label="Baseline AUC")
...
if __name__ == "__main__":
    ...
    ridge_prob, ridge_train_auc, ridge_test_auc = evaluate_model(
        ridge, X_train, y_train, X_test, y_test, "Ridge (L2 Regularization)"
    )
    lasso_prob, lasso_train_auc, lasso_test_auc = evaluate_model(
        lasso, X_train, y_train, X_test, y_test, "Lasso (L1 Regularization)"
    )
    knn_prob, knn_train_auc, knn_test_auc = evaluate_model(
        knn, X_train, y_train, X_test, y_test, f"KNN (k={best_k})"
    )
    ...
    results = [
        ...
        {"model": "Ridge",  "train_auc": round(ridge_train_auc, 4),
         "test_auc": round(ridge_test_auc, 4), "notes": f"L2, C={best_c_ridge}"},
        {"model": "Lasso",  "train_auc": round(lasso_train_auc, 4),
         "test_auc": round(lasso_test_auc, 4), "notes": f"L1, C={best_c_lasso}"},
        {"model": f"KNN",   "train_auc": round(knn_train_auc, 4),
         "test_auc": round(knn_test_auc, 4), "notes": f"k={best_k}, distance weights"},
    ]
