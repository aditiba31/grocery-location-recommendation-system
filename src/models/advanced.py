...
# Run: python src/models/advanced.py
...
def tune_random_forest(X_train, y_train):
    ...
    for params in param_grid:
        ...
        mean   = scores.mean()
        print(f"  RF {params} -> CV AUC={mean:.4f}")
        if mean > best_score:
            best_score  = mean
            best_params = params

    print(f"  Best RF: {best_params}  CV AUC={best_score:.4f}")
    return best_params
...
def tune_gradient_boosting(X_train, y_train):
    ...
    for params in param_grid:
        ...
        scores = cross_val_score(model, X_train, y_train, cv=cv, scoring="roc_auc")
        mean   = scores.mean()
        print(f"  GB {params} -> CV AUC={mean:.4f}")
        if mean > best_score:
            best_score  = mean
            best_params = params

    print(f"  Best GB: {best_params}  CV AUC={best_score:.4f}")
    return best_params
...
def plot_roc_all(X_test, y_test, rf_model, gb_model, rf_auc, gb_auc):
    ...
    if baseline_path.exists():
        ...
        ax.plot(fpr, tpr, color=colors[0], linewidth=2,
                label=f"Logistic Regression (AUC={base_auc:.4f})")
    ...
    if ridge_path.exists():
        ...
        ax.plot(fpr, tpr, color=colors[1], linewidth=2,
                label=f"Ridge (AUC={ridge_auc:.4f})")
    ...
    fpr, tpr, _ = roc_curve(y_test, y_prob)
    ax.plot(fpr, tpr, color=colors[3], linewidth=2,
...
def save_results(results, rf_model, gb_model,
...
if __name__ == "__main__":
    ...
    gb_prob, gb_train_auc, gb_test_auc = evaluate_model(
    ...
    results = [
        {"model": "Logistic Regression", "train_auc": 0.8844,
         "test_auc": 0.8467, "notes": "Baseline"},
        {"model": "Ridge",               "train_auc": 0.8848,
         "test_auc": 0.8478, "notes": "L2, C=10"},
        {"model": "Lasso",               "train_auc": 0.8780,
         "test_auc": 0.8363, "notes": "L1, C=0.1"},
        {"model": "KNN",                 "train_auc": 1.0000,
         "test_auc": 0.4923, "notes": "k=21, failed"},
        {"model": "Random Forest",       "train_auc": round(rf_train_auc, 4),
         "test_auc": round(rf_test_auc, 4), "notes": str(rf_params)},
        {"model": "Gradient Boosting",   "train_auc": round(gb_train_auc, 4),
         "test_auc": round(gb_test_auc, 4), "notes": str(gb_params)},
    ]

    plot_roc_all(X_test, y_test, rf_model, gb_model,
                 rf_test_auc, gb_test_auc)

    plot_final_comparison(results)

    save_results(results, rf_model, gb_model,
                 rf_importance, gb_importance)
